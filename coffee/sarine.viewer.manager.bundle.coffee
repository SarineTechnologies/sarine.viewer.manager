...
class ViewerManger
	...
	loadTemplate = (selector) -> 
		...
		deferArr = []
		scripts = []
		$("<div>").load(template + "?" + cacheVersion, (a,b,c)-> 
	...
	addViewer = (type,toElement)->
		...
		$.getJSON jsons.replace("{version}",toElement.data("version") || "v1") + type + ".json?" + cacheVersion ,(d)=>
			data = d;
		$.ajaxSetup(
			async : true
		); 
		...
		url = logicRoot.replace("{version}", toElement.data("version") || "v1") + data.name + (if location.hash.indexOf("debug") == 1 then ".bundle.js" else ".bundle.min.js") + "?" + cacheVersion
		# $.getScript(url,()->
