###!
sarine.viewer.manager - v0.21.0 -  Sunday, April 28th, 2019, 4:01:26 PM 
...
class ViewerManger
	...
	bindElementToSelector = (selector)->
		...
		$(selector).find(fromTag).each((i, v) =>
			...
			exp = configuration.experiences.filter (i) -> i.atom ==type
			order = if exp && exp.length>0  then exp[0].order else 99
